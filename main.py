...
from app.RebalStrategy import RebalanceStrategy, printTradeAnalysis, AcctValue, saveplots
...
def main():
    ...
    plt.legend()
    plt.savefig("PortfolioLogReturns.png")
    plt.show()
    ...
    plt.ylabel("Dollars")
    plt.legend()
    plt.savefig("DollarPortfolioValue.png")
    plt.show()
...
def main_old():
    ...
    cerebro.addanalyzer(bta.PyFolio)
    cerebro.broker.set_checksubmit(False)
    ...
    print("Portfolio Total Log Returns: {}".format(tot_returns["rtot"]))
    print("Portfolio Sharpe Ratio: {}".format(sharpe['sharperatio']))
    print("Portfolio Max Drawdown: ${}, Time: {} days".format(max_drawdown.max["moneydown"], max_drawdown.max["len"]))
    print("---Compare to SPY---")
    print("SPY Portfolio Value: ${}".format(SPY_portfolio_values[-1]))
    print("SPY Portfolio Pnl: ${}".format(SPY_portfolio_values[-1] - startcash))
    print("SPY Total Log Returns: {}".format(SPY_returns[-1]))
